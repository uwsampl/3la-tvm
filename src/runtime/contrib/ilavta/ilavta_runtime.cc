 ...
 public:
  ...
  void Run() override {
    ...
    if (outputs_.size() == 1 && nodes_[outputs_[0].id_].GetOpName() == "ilavta.dense") {
      ...
      int in_dim = n_inp_cols % VTA_BLOCK_IN != 0 ? n_inp_cols / VTA_BLOCK_IN + 1
                                                  : n_inp_cols / VTA_BLOCK_IN;
      int out_dim = n_wgt_rows % VTA_BLOCK_OUT != 0 ? n_wgt_rows / VTA_BLOCK_OUT + 1
                                                    : n_wgt_rows / VTA_BLOCK_OUT;
      ...
      VTAGenericInsn* instrs =
          static_cast<VTAGenericInsn*>(VTAMemAlloc(sizeof(VTAGenericInsn) * num_instr, 0));

      int8_t* input_buf =
          reinterpret_cast<int8_t*>(VTAMemAlloc(sizeof(int8_t) * batch * in_channels, 0));
      int8_t* wgt_buf =
          reinterpret_cast<int8_t*>(VTAMemAlloc(sizeof(int8_t) * out_channels * in_channels, 0));
      int32_t* acc_buf =
          reinterpret_cast<int32_t*>(VTAMemAlloc(sizeof(int32_t) * batch * out_channels, 0));
      VTAUop* uop_buf =
          getGEMMUops(batch / VTA_BATCH, in_channels / VTA_BLOCK_IN, out_channels / VTA_BLOCK_OUT);
      int8_t* out_buf =
          reinterpret_cast<int8_t*>(VTAMemAlloc(sizeof(int8_t) * out_channels * batch, 0));
      ...
      instrs[ptr++] =
          get1DLoadStoreInsn(VTA_OPCODE_LOAD, VTA_MEM_ID_UOP, 0,
                             VTAMemGetPhyAddr(uop_buf) / VTA_UOP_ELEM_BYTES, uop_size, 0, 0, 0, 0);

      instrs[ptr++] = get1DLoadStoreInsn(VTA_OPCODE_LOAD, VTA_MEM_ID_ACC, 0,
                                         VTAMemGetPhyAddr(acc_buf) / VTA_ACC_ELEM_BYTES,
                                         output_size, 0, 0, 1, 0);

      instrs[ptr++] =
          get1DLoadStoreInsn(VTA_OPCODE_LOAD, VTA_MEM_ID_WGT, 0,
                             VTAMemGetPhyAddr(wgt_buf) / VTA_WGT_ELEM_BYTES, wgt_size, 0, 1, 0, 0);

      instrs[ptr++] = get1DLoadStoreInsn(VTA_OPCODE_LOAD, VTA_MEM_ID_INP, 0,
                                         VTAMemGetPhyAddr(input_buf) / VTA_INP_ELEM_BYTES,
                                         input_size, 0, 0, 0, 1);

      instrs[ptr++] = getGEMMInsn(0, batch / VTA_BATCH, in_channels / VTA_BLOCK_IN,
                                  out_channels / VTA_BLOCK_OUT, 0, 1, 0, 0, 1);

      instrs[ptr++] = get1DLoadStoreInsn(VTA_OPCODE_STORE, VTA_MEM_ID_OUT, 0,
                                         VTAMemGetPhyAddr(out_buf) / VTA_OUT_ELEM_BYTES,
                                         output_size, 1, 0, 1, 0);
      ...
      CHECK(output_data->ndim == 2) << "Output dimension error: "
                                    << "expected 2, actual " << output_data->ndim;
      ...
      CHECK(out_values.size() == static_cast<size_t>(output_size * VTA_BLOCK_OUT))
          << "Output element size mismatch: " << output_size * VTA_BLOCK_OUT << " v.s. "
          << buf_size;
      ...
      CHECK(out_w == static_cast<size_t>(n_wgt_rows))
          << "Dimension mismatch: " << out_w << "; expected " << n_wgt_rows;

      size_t bufsize_read = loadILAOutput(out_values, buffer, out_h, out_w);

      CHECK(bufsize_read == buf_size)
          << "Number read differs from expected buffer size: " << bufsize_read << " v.s. "
          << buf_size;
      ...
      int in_feat = n_inp_cols % VTA_BLOCK_OUT == 0 ? n_inp_cols / VTA_BLOCK_OUT
                                                    : n_inp_cols / VTA_BLOCK_IN + 1;
      ...
      int32_t* input_buf =
          reinterpret_cast<int32_t*>(VTAMemAlloc(sizeof(int32_t) * batch * in_channels, 0));
      // TVM does array broadcasting over the matrix in bias_add
      int32_t* bias_buf =
          reinterpret_cast<int32_t*>(VTAMemAlloc(sizeof(int32_t) * 1 * bias_channels, 0));
      int8_t* out_buf =
          reinterpret_cast<int8_t*>(VTAMemAlloc(sizeof(int8_t) * batch * in_channels, 0));
      VTADeviceHandle device = VTADeviceAlloc();
      ...
      instrs[ptr++] = get1DLoadStoreInsn(VTA_OPCODE_LOAD, VTA_MEM_ID_ACC, input_size,
                                         VTAMemGetPhyAddr(bias_buf) / VTA_ACC_ELEM_BYTES,
                                         output_size, 0, 0, 0, 0);

      instrs[ptr++] = get1DLoadStoreInsn(VTA_OPCODE_LOAD, VTA_MEM_ID_ACC, 0,
                                         VTAMemGetPhyAddr(input_buf) / VTA_ACC_ELEM_BYTES,
                                         input_size, 0, 0, 0, 0);

      instrs[ptr++] = getAluInsn(VTA_ALU_OPCODE_ADD, 0, uop_size, false, 0, 0, 0, 0, 1);
      ...
      runSimGetData("ilavta_bias_add", output_buffer_size, n_inp_rows, n_inp_cols,
                    output_data->data);
      ...
      VTADeviceHandle device = VTADeviceAlloc();
      VTAGenericInsn* instrs =
          static_cast<VTAGenericInsn*>(VTAMemAlloc(sizeof(VTAGenericInsn) * num_instrs, 0));
      int32_t* input_buf =
          reinterpret_cast<int32_t*>(VTAMemAlloc(sizeof(int32_t) * batch * in_channels, 0));
      ...
      instrs[ptr++] = get1DLoadStoreInsn(VTA_OPCODE_STORE, VTA_MEM_ID_OUT, 0,
                                         VTAMemGetPhyAddr(input_buf) / VTA_OUT_ELEM_BYTES,
                                         sim_output_size, 1, 0, 1, 0);
...
TVM_REGISTER_GLOBAL("runtime.ILAVTARuntimeCreate").set_body_typed(ILAVTARuntimeCreate);
