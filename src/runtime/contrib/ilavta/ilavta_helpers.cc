...
json get_blocked_gemm(int batch, int channels, int block, bool uop_compression, int virtual_threads,
                      int factor, int nbits) {
  // Some assertions
  ...
  int uop_size = uop_compression ? block / VTA_BATCH * virtual_threads
                                 : block / VTA_BATCH * block / VTA_BLOCK_IN * block /
                                       VTA_BLOCK_OUT * virtual_threads;
  ...
  VTAGenericInsn* insn_buf =
      static_cast<VTAGenericInsn*>(VTAMemAlloc(sizeof(VTAGenericInsn) * ins_size, 0));
  int insn_idx = 0;
  ...
  //                                           0,
  //                                           0);
  prog.push_back(get2DLoadStoreAsm(VTA_OPCODE_LOAD, VTA_MEM_ID_UOP, 0, 0, 1, uop_size, 0, 0, 0));
  ...
  for (int i = 0; i < batch; i += block) {
    ...
    for (int j = 0; j < out_feat; j += block) {
      ...
      prog.push_back(get2DLoadStoreAsm(
          VTA_OPCODE_LOAD, VTA_MEM_ID_ACC, 0, (i / VTA_BATCH * out_feat + j) / VTA_BLOCK_OUT,
          block / VTA_BATCH, block / VTA_BLOCK_OUT, out_feat / VTA_BLOCK_OUT, 0, 0));
      // Iterate over input channel blocks
      for (int k = 0; k < in_feat; k += block) {
        for (int l = 0; l < block; l += block) {
          ...
          //     0);                                             // push next dep
          prog.push_back(get2DLoadStoreAsm(
              VTA_OPCODE_LOAD, VTA_MEM_ID_WGT, l / VTA_BLOCK_IN * block / VTA_BLOCK_OUT,
              (j / VTA_BLOCK_OUT * in_feat + k + l) / VTA_BLOCK_IN, block / VTA_BLOCK_OUT,
              block / VTA_BLOCK_IN, in_feat / VTA_BLOCK_IN, 0, 0));
          ...
          //     1);                                             // push next dep
          prog.push_back(get2DLoadStoreAsm(
              VTA_OPCODE_LOAD, VTA_MEM_ID_INP, l / VTA_BLOCK_IN * block / VTA_BATCH,
              (i / VTA_BATCH * in_feat + k + l) / VTA_BLOCK_IN, block / VTA_BATCH,
              block / VTA_BLOCK_IN, in_feat / VTA_BLOCK_IN, 0, 0));
          ...
          prog.push_back(getGEMMAsm(l / block * uop_size, block / VTA_BATCH, block / VTA_BLOCK_IN,
                                    block / VTA_BLOCK_OUT));
        }
      }
      ...
      //       0);                                                 // push next dep
      // }
      prog.push_back(getAluAsm(4, 0, uop_size, 1, factor));
      ...
      prog.push_back(get2DLoadStoreAsm(
          VTA_OPCODE_STORE, VTA_MEM_ID_OUT, 0, (i / VTA_BATCH * out_feat + j) / VTA_BLOCK_OUT,
          block / VTA_BATCH, block / VTA_BLOCK_OUT, out_feat / VTA_BLOCK_OUT, 0, 0));
    }
    return prog_frag;
  }
